...
def excluir_recibo():
    ...
    tk.Label(senha_janela, text="Digite a senha para excluir o recibo:").pack(padx=10, pady=10)
...
def abrir_janela_inclusao():
    ...
    janela_inclusao.geometry("620x650")  # Ajuste no tamanho da janela para permitir mais espaço
...
def editar_cliente(janela_consulta, tree, cliente_id):
    ...
    janela_edicao.geometry("460x620")  # Aumentei o tamanho para caber todos os campos
...
# Campos de valores
    ...
    try:
        ...
        if cliente:
            # Preencher os campos com os dados do cliente, verificando se são NULL
            entry_nome.insert(0, cliente[0] or "")
            entry_fantasia.insert(0, cliente[1] or "")
            entry_cpfcnpj.insert(0, cliente[2] or "")
            entry_telefone.insert(0, cliente[3] or "")
            entry_celular.insert(0, cliente[4] or "")
            entry_email.insert(0, cliente[5] or "")
            entry_endereco.insert(0, cliente[6] or "")
            entry_numero.insert(0, cliente[7] or "")
            entry_bairro.insert(0, cliente[8] or "")
            entry_cidade.insert(0, cliente[9] or "")
            entry_aluguel.insert(0, cliente[10] or "")
            entry_agua.insert(0, cliente[11] or "")
            entry_luz.insert(0, cliente[12] or "")
            entry_condominio.insert(0, cliente[13] or "")
            entry_internet.insert(0, cliente[14] or "")
            entry_iptu.insert(0, cliente[15] or "")
            entry_limpeza.insert(0, cliente[16] or "")
            entry_outros.insert(0, cliente[17] or "")
    ...
    # Verificação de obrigatoriedade apenas para Nome e CPF/CNPJ
        ...
        try:
            ...
            WHERE id = %s
        """, (nome, fantasia, cpfcnpj, telefone, celular, email, endereco, numero, bairro, cidade, aluguel, 
              (agua if agua else None), (luz if luz else None), (condominio if condominio else None), 
              (internet if internet else None), (iptu if iptu else None), (limpeza if limpeza else None), 
              (outros if outros else None), cliente_id))
            conn.commit()
            conn.close()
    ...
    # Botões de salvar, cancelar e fechar
    btn_salvar = tk.Button(janela_edicao, text="Salvar", command=salvar_edicao, bg="blue", fg="white")
    btn_salvar.grid(row=18, column=1, padx=10, pady=20)

    # Novo botão Fechar
    btn_fechar = tk.Button(janela_edicao, text="Fechar", command=janela_edicao.destroy, bg="green", fg="white")
    btn_fechar.grid(row=18, column=0, padx=10, pady=20)
