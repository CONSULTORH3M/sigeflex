...
########################################################################## SALVAR EDICAO
        def salvar_edicao():
            ...
            try:
                ...
                c.execute(''' 
                    UPDATE dados
                    SET nome=?, cnpj=?, valor=?, telefone=?, formaPag=?, referente=?, observacao=?, dataEmissao=?
                    WHERE id_recibo=?
                ''', (nome, cnpj, valor, telefone, forma_pagamento, referente, observacao, data_emissao, id_recibo))
...
#######################################################################################BIBLIOTECAS PARA GERAR RELATORIO E PDF        
from reportlab.lib.pagesizes import letter
from reportlab.pdfgen import canvas
import sqlite3
import os
