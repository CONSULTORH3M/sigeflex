...
def abrir_janela_inclusao():
    ...
    janela_inclusao.geometry("500x520")  # Aumentando o tamanho da janela, como no código de edição
...
##################################################### BOTOES QUE FICA DENTRO DA JANELA INCLUIR - SALVAR RECIBO BANCO DADOS ###########
    ...
    btn_pdf = tk.Button(janela_inclusao, text="Imprimir_PDF", command=imprimir_recibo_selecionado, bg="blue", fg="white", relief="raised", padx=10, pady=5)
...
def editar_recibo():
    try:
        ...
        janela_editar.resizable(False, False)  # Bloqueia o redimensionamento da janela
        ...
        btn_salvar_edicao.grid(row=len(campos), column=0, columnspan=2, pady=10)
...
########## Solicitar o ID do recibo via a nova função########################################################
    ...
    # Quando o usuário fornece o número, o Tkinter é fechado e o número é retornado
            ...
            if numero_recibo:  # Se o usuário não cancelar ou deixar em branco
                print(f"Número do recibo: {numero_recibo}")
                return numero_recibo
            # vou justificar para não ficar imprimindo e pode seer isso quee feche a janela principal do sistema
            #else:
                #print("Nenhum número de recibo informado.")
                #return None
        ...
        if not id_recibo:
            messagebox.showwarning("Atenção", "Nenhum Recibo Selecionado. Favor Selecionar na GRID.")
            return
...
btn_filtro = tk.Button(janela_principal, text="FILTRAR POR DATA", command=gerar_relatorio_filtrado, bg="purple", fg="white") # FILTRO POR DATA
btn_filtro.grid(row=3, column=2, padx=10, pady=10)

# Botão Gerar RELATORIO = FUNCAO GERAR_REL  LINHA 389
btn_rel = tk.Button(janela_principal, text="Relatorio GERAL-Todos", command=gerar_Rel, bg="purple", fg="white") # traz todos os recibos da base
